...
class Base {
    ...
    getCount(){
        ...
        if(count ===0){
            tpl=`您选了 <b class="em" id="count">${count}</b> 注，共 <b class="em" id="money">${count*2}</b> 元`
        }else if(range[0] === range[1]){
            tpl=`您选了 <b class="em" id="count">${count}</b> 注，共 <b class="em" id="money">${count*2}</b> 元,若中奖，奖金：
    ...
    getRandom(num){
        let arr=[],index;
        let number = Array.from(this.number);//把类似数组的集合转成数组
        while(num--){
